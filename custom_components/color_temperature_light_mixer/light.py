...
async def async_setup_entry(
    ...
    light = TemperatureMixerLight(
        ...
        warm_light={
            ...
            ATTR_COLOR_TEMP_KELVIN: int(config[CONF_WARM_LIGHT_TEMPERATURE_KELVIN]),
        },
        cold_light={
            CONF_ENTITY_ID: config[CONF_COLD_LIGHT],
            ATTR_COLOR_TEMP_KELVIN: int(config[CONF_COLD_LIGHT_TEMPERATURE_KELVIN]),
        },
        config_id=entry.entry_id,
        mixing_mode=MixingMode(entry_priority(entry)),
    )
...
class TemperatureMixerLight(LightGroup, RestoreSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Hide member entity IDs so Adaptive Lighting does not treat this as a group.

        LightGroup publishes entity_id=[WW, CW]. Adaptive Lighting then expands the
        mixer to those channels and sets both to the same brightness, which forces
        the midpoint kelvin (~3824K).
        """
        return {}
    ...
    def async_update_group_state(self) -> None:
        ...
        self._attr_min_color_temp_kelvin = self.warm_light[ATTR_COLOR_TEMP_KELVIN]
        self._attr_max_color_temp_kelvin = self.cold_light[ATTR_COLOR_TEMP_KELVIN]
        ...
        elif self.mixing_mode == MixingMode.COLOR_TEMPERATURE:
            # 100% is the strongest channel at this kelvin (WW-only at 2700K is 100%).
            self._attr_brightness = int(max(brightnesses))
        else:
            self._attr_brightness = int(sum(brightnesses) / 2)
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        common_data = {
            ...
            and key not in (ATTR_BRIGHTNESS, ATTR_COLOR_TEMP_KELVIN)
        }
        ...
        # Clamp / snap to the configured WW–CW rails. HA floors kelvin through
        # mireds (2700K → 370 mired → 2702K), so treat values within 2K of a
        # rail as that rail.
        target_temp_kelvin = self._snap_kelvin(target_temp_kelvin)
    ...
    def _snap_kelvin(self, kelvin: int) -> int:
        """Clamp kelvin to the WW/CW range, snapping mired-rounding onto the rails."""
        warm = int(self.warm_light[ATTR_COLOR_TEMP_KELVIN])
        cold = int(self.cold_light[ATTR_COLOR_TEMP_KELVIN])
        kelvin = int(kelvin)
        if kelvin <= warm + 2:
            return warm
        if kelvin >= cold - 2:
            return cold
        return min(cold, max(kelvin, warm))
    ...
    async def _turn_on_lights(
        ...
        for light in (ww, cw):
            ...
            turn_off = (
                self.mixing_mode == MixingMode.COLOR_TEMPERATURE
                and light.brightness is not None
                and light.brightness <= 0
            )
            if turn_off:
                _LOGGER.debug(
                    "%s: forward turn_off unused channel: %s",
                    self._cached_friendly_name,
                    target,
                )
            ...
            service_data = dict(light.common_data)
